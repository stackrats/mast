...
7. **Stale Desktop context, live specimen:** `desktop-linux` exists with `DockerEndpoint: unix:///home/<user>/.docker/desktop/docker.sock`, `Error: ""` in `ls` — but the socket doesn't exist. `ls` output alone looks healthy; only dialing fails. Error strings are clean and parseable:
